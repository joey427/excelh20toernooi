...
echo [1/13] Tijdelijke bestanden gebruiker (%TEMP%)...
if exist "%TEMP%" (
    rd /s /q "%TEMP%" 2>nul
    md "%TEMP%" 2>nul
)
...
:: --- Windows Temp map ---
echo [2/13] Windows Temp map (C:\Windows\Temp)...
...
echo [3/13] Prefetch bestanden...
if exist "C:\Windows\Prefetch" (
    del /f /q "C:\Windows\Prefetch\*.*" 2>nul
)

:: --- Recente bestanden ---
echo [4/13] Recente bestanden...
...
echo [7/13] Microsoft Edge cache...
...
:: --- Microsoft Excel: factory reset (account blijft bewaard) ---
echo [9/13] Excel afsluiten...
taskkill /f /im excel.exe 2>nul

echo [10/13] Excel factory reset (inloggegevens blijven bewaard)...
...
:: --- Office cache ---
echo [11/13] Office cache (OfficeFileCache) wissen...
...
echo [12/13] Prullenbak leegmaken...
rd /s /q "C:\$Recycle.Bin" 2>nul

:: --- Windows taal instellen op Engels (United States) ---
echo [13/13] Windows taal instellen op Engels (United States)...
